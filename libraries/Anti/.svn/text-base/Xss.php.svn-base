...
class Anti_Xss {
    ...
    public function __construct($filename = 'Rule.php') {
        ...
        if (file_exists($file)) {
            ...
            require_once ($file);
            $this->config = $_;
            $key_array = array('black_dom', 'black_attr', 'white_dom', 'white_attr', 'filter_dom', 'filter_attr');
    ...
    private function check_attr_rule(&$node, $key, $rules) {
        ...
        //过滤各种注释
        while ($_value = preg_replace("/\/\*.*?\*\//s", '', $value)) {
            if ($_value == $value) {
                break;
            }
            $value = $_value;
        ...
        //过滤各种进制的编码
        while ($_value = str_replace("\\00", '', $value)) {
            if ($_value == $value) {
                break;
            }
            $value = $_value;
        ...
        while ($_value = substr($value, 1)) {
            ...
            //assic特殊字符
            if (!$this->is_anti_ascii(ord($f_char))) {
                break;
            }
            $value = $_value;
        }
        ...
        while ($_value = substr($value, 0, -1)) {
            ...
            //assic特殊字符
            if (!$this->is_anti_ascii(ord($e_char))) {
                break;
            }
            $value = $_value;
        }
