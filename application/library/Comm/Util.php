...
class Comm_Util {
   ...
   /**
    ...
    public static function getRealClientIp($to_long = false) {
        ...
        if (Comm_Util::is_private_ip(self::getServer('REMOTE_ADDR')) && isset($proxied_client_ip)) {
    ...
    */
    ...
    public static function getCurrentUrl($urlencode = true) {
        ...
        $port = ($port == '80') ? '' : (':' . $port);
        if(self::getServer('HTTP_HOST')){
            $current_url = $protocol . '://' . self::getServer('HTTP_HOST') . $req_uri;
        }else{
            $current_url = $protocol . '://' . self::getServer('SERVER_NAME') . $port . $req_uri;
        }
